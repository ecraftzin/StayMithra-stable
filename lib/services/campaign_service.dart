...
class CampaignService {
  ...
  Future<List<CampaignModel>> getUserCampaigns(
    ...
    try {
      // Use a more explicit query with additional filtering
      final response = await _supabase
          .from('campaigns')
          .select('''
            id,
      ...
      // Parse and double-filter the results
      final campaigns = <CampaignModel>[];
      for (final item in response) {
        try {
          final campaign = CampaignModel.fromJson(item);
