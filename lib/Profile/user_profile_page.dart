  ...
  Future<void> _handleFollowAction() async {
    ...
    try {
      switch (_followStatus) {
        case 'not_following':
        case 'followed_by':
          // For direct follow (no follow requests in this case)
          final result = await followService.followUser(currentUser.id, widget.userId);
          ...
          if (success) {
            setState(() {
              ...
              _followersCount = result['followersCount'] ?? _followersCount;
            });
          }
          break;

        case 'requested':
          success = await _followRequestService.cancelFollowRequest(widget.userId);
        ...
        case 'following':
        case 'mutual':
          // Use the improved unfollow method
          final result = await followService.unfollowUser(currentUser.id, widget.userId);
          success = result['success'] ?? false;
