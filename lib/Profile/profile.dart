...
class _ProfilePageState extends State<ProfilePage> with TickerProviderStateMixin {
  ...
  Future<void> _loadUserStats(String userId) async {
    try {
      ...
      // Load follower counts using the centralized service
      await _loadFollowerCounts(userId);
    } catch (e) {
      // ignore: avoid_print
  ...
  Future<void> _loadFollowerCounts(String userId) async {
    try {
      // Use the centralized FollowService for consistent counting
      final followersCount = await _followService.getFollowersCount(userId);
      final followingCount = await _followService.getFollowingCount(userId);
          ...
          children: [
            ...
            Padding(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    Expanded(child: _ProfileStat(title: "Following", count: "$_followingCount")),
                    SizedBox(height: 24, child: VerticalDivider(thickness: 1, color: Colors.black12)),
                    Expanded(child: _ProfileStat(title: "Posts", count: "${_userPosts.length + _userCampaigns.length}")),
                    SizedBox(height: 24, child: VerticalDivider(thickness: 1, color: Colors.black12)),
                    Expanded(child: _ProfileStat(title: "Followers", count: "$_followersCount")),
                  ],
                ),
              ),
            ),
  ...
  PreferredSizeWidget _buildTabBar() {
    ...
    return TabBar(
      ...
      tabs: [
        Tab(icon: Icon(Icons.apps, size: screenWidth * 0.045), text: 'All'),
        Tab(icon: Icon(Icons.grid_on, size: screenWidth * 0.045), text: 'Posts'),
        Tab(icon: Icon(Icons.campaign, size: screenWidth * 0.045), text: 'Campaigns'),
      ],
    );
  }
  ...
  Widget _buildPostsGrid() {
    ...
    return GridView.builder(
      ...
      physics: const ClampingScrollPhysics(), // important inside NestedScrollView
              ...
              child: PopupMenuButton<String>(
                ...
                itemBuilder: (_) => const [
                  ...
                  PopupMenuItem(
                    value: 'share',
                    child: Row(
                      children: [Icon(Icons.share, size: 16), SizedBox(width: 8), Text('Share')],
                    ),
                  ),
                ],
              ),
  ...
  void _showContentDetail(dynamic item, bool isPost) {
    if (isPost) {
      Navigator.push(context, MaterialPageRoute(builder: (_) => PostDetailPage(post: item)));
    } else {
      Navigator.push(context, MaterialPageRoute(builder: (_) => CampaignDetailPage(campaign: item)));
    }
  }
  ...
  void _editContent(dynamic item, bool isPost) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Edit ${isPost ? 'post' : 'campaign'}: ${item.id}')),
    );
  }
  ...
  void _deleteContent(dynamic item, bool isPost) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete this ${isPost ? 'post' : 'campaign'}?'),
        actions: [
  ...
  Future<void> _performDelete(dynamic item, bool isPost) async {
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${isPost ? 'Post' : 'Campaign'} deleted successfully'),
            backgroundColor: Colors.green),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error deleting ${isPost ? 'post' : 'campaign'}: $e'),
            backgroundColor: Colors.red),
      );
    }
  }
  ...
  Future<void> _shareContent(dynamic item, bool isPost) async {
    try {
      ...
      final shareUrl = 'https://staymitra.app/${isPost ? 'post' : 'campaign'}/${item.id}';
