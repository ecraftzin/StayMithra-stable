...
class _UserSearchPageState extends State<UserSearchPage> {
  ...
  Future<void> _startChat(UserModel user) async {
    ...
    try {
      // Check if both users follow each other using direct database queries
      final user1FollowsUser2 = await supabase
          .from('follows')
          .select('id')
      ...
      final user2FollowsUser1 = await supabase
          .from('follows')
          .select('id')
      ...
      final hasMutualFollow =
          user1FollowsUser2 != null && user2FollowsUser1 != null;
      ...
      final chat = await _chatService.createOrGetChat(currentUser.id, user.id);
      ...
      switch (currentStatus) {
        ...
        case 'incoming_request':
          ...
          if (requestId != null) {
            ...
            if (success) {
              // Update the status immediately to avoid confusion
              _followStatuses[user.id] = 'followed_by';
    ...
    if (_searchResults.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.search_off, size: screenWidth * 0.2, color: Colors.grey),
            SizedBox(height: screenHeight * 0.02),
            Text(
              'No users found',
              style: TextStyle(
      ...
      child: ListTile(
        ...
        onLongPress: () {
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) =>
                  UserProfilePage(userId: user.id, userName: user.username),
            ),
          );
        },
